...
class Trades(object):
    ...
    def load(self):
        ...
        while self.__len == self.__limit:
            t = trades(engine=self.__engine, market=self.__market, boardid=self.__boardid,
                securityid=self.__securityid, limit=self.__limit, start=self.__start)
            data.append(t)
...
------------------------------------------------------------------'''.format(
            ...
            self.__trades.shape[0], self.__trades['secid'].nunique(), self.__trades['systime'].max())